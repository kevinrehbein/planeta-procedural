...
async function main() {
  ...
  const lightViewProjectionLocation = gl.getUniformLocation(
    programPlanet,
  ...
  const programWater = gl.createProgram();

  const waterVertexShader = gl.createShader(gl.VERTEX_SHADER);
  gl.shaderSource(waterVertexShader, vsWater);
  gl.compileShader(waterVertexShader);
  gl.attachShader(programWater, waterVertexShader);

  const waterFragmentShader = gl.createShader(gl.FRAGMENT_SHADER);
  gl.shaderSource(waterFragmentShader, fsWater);
  gl.compileShader(waterFragmentShader);
  gl.attachShader(programWater, waterFragmentShader);

  gl.linkProgram(programWater);
  if (!gl.getProgramParameter(programWater, gl.LINK_STATUS)) {
    console.log(gl.getShaderInfoLog(waterVertexShader));
    console.log(gl.getShaderInfoLog(waterFragmentShader));
  }
  gl.useProgram(programPlanet);

  var vaoWater = gl.createVertexArray();
  const resWater = 100;
  const waterSphere = createBaseSphere(resWater);
  ...
  gl.bufferData(
    gl.ARRAY_BUFFER,
    new Float32Array(waterSphere.vertices),
    gl.STATIC_DRAW,
  );
  gl.vertexAttribPointer(positionAttributeLocation, 3, gl.FLOAT, false, 0, 0);
  gl.enableVertexAttribArray(positionAttributeLocation);

  var aguaIndexBuffer = gl.createBuffer();
  gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, aguaIndexBuffer);
  gl.bufferData(
    gl.ELEMENT_ARRAY_BUFFER,
    new Uint32Array(waterSphere.indices),
    gl.STATIC_DRAW,
  );
  ...
  const models = {
    tree: await loadOBJ(
      gl,
      "./Assets/obj/Tree_1_A_Color1.obj",
      vsObject,
      fsObject,
    ),
    grass: await loadOBJ(
      gl,
      "./Assets/obj/Grass_2_C_Color1.obj",
      vsObject,
      fsObject,
    ),
    rock: await loadOBJ(
      gl,
      "./Assets/obj/Rock_3_A_Color1.obj",
      vsObject,
      fsObject,
    ),
  ...
  function render(time) {
    ...
    const lightProjection = m4.orthographic(-4, 4, -4, 4, 0.1, 8);
    ...
    gl.uniform3fv(
      ...
      normalizeVec3(reverseLightDirection),
    );
    gl.uniform1i(shadowMapLocation, 0);
    gl.uniformMatrix4fv(
      lightViewProjectionLocation,
      false,
      lightViewProjection,
    );
    ...
    gl.enable(gl.BLEND);
    gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
    // desativar a escrita no depth buffer para evitar artefactos se houver múltiplas camadas
    //gl.depthMask(false);

    gl.useProgram(programWater);
    gl.bindVertexArray(vaoWater);

    const waterLoc = (name) => gl.getUniformLocation(programWater, name);

    gl.uniformMatrix4fv(waterLoc("u_world"), false, modelMatrix);
    gl.uniformMatrix4fv(
      waterLoc("u_worldInverseTranspose"),
      false,
      modelInverseTransposeMatrix,
    );
    gl.uniformMatrix4fv(waterLoc("u_projection"), false, projection);
    gl.uniformMatrix4fv(waterLoc("u_view"), false, view);
    gl.uniform3fv(waterLoc("u_cameraPosition"), cameraPosition);
    gl.uniform3fv(
      waterLoc("u_reverseLightDirection"),
      normalizeVec3(reverseLightDirection),
    );
    gl.uniform1i(waterLoc("u_shadowMap"), 0);
    gl.uniformMatrix4fv(
      waterLoc("u_lightViewProjection"),
      false,
      lightViewProjection,
    );
    gl.uniform1f(waterLoc("u_seaLevel"), seaLevel);

    gl.drawElements(
      gl.TRIANGLES,
      waterSphere.indices.length,
      gl.UNSIGNED_INT,
      0,
    );
    ...
    for (const instance of objectInstances) {
      ...
      for (const { bufferInfo, vao, material } of modelData.parts) {
        ...
        const diffuse = material.diffuse ? [...material.diffuse] : [1, 1, 1];
        ...
        let modelMatrix = m4.multiply(rotation, model);

        let modelInverseMatrix = m4.inverse(modelMatrix);
        let modelInverseTransposeMatrix = m4.transpose(modelInverseMatrix);
